...
#define INTERNAL_CATCH_TEST( macroName, resultDisposition, ... ) \
    ...
    } while( (void)0, false && static_cast<bool>( !!(__VA_ARGS__) ) ) // the expression here is never evaluated at runtime but it force the compiler to give it a look
    // The double negation silences MSVC's C4800 warning, the static_cast force short-circuit evaluation if the type has overloaded &&.
